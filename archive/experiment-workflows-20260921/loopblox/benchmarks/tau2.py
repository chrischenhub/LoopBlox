...
def prepare_suite(output, *, source, development=18, holdout=18, seed=1701, exclude_suites=()):
    ...
    reserved = {(task["family"], task["group"]) for path in exclude_suites
                for task in json.loads((Path(path) / "manifest.json").read_text())["tasks"]}
    rng, selected, audit = random.Random(seed), [], {}
    for domain in DOMAINS:
        raw = json.loads((source / "data/tau2/domains" / domain / "tasks.json").read_text())
        official = json.loads((source / "data/tau2/domains" / domain / "split_tasks.json").read_text())
        ...
        for group in groups:
            roles = [role for role in ("train", "test") if all(task.id in official[role] for task in group)]
            if not roles:
                excluded["group_crosses_official_split"] += 1
                continue
            split = "development" if roles[0] == "train" else "holdout"
            ...
            pools[split, stratum].append((task, group_id))
        ...
        targets = dict(development=development, holdout=holdout)
        ...
        while len(accepted) < development + holdout and any(pools.values()):
            ...
            for split, stratum in strata:
                if not pools[split, stratum] or counts[split] >= targets[split]:
                    continue
                progressed = True
                task, group_id = pools[split, stratum].pop()
                try:
                ...
                if empty.reward == 1:
                    ...
                    continue
                accepted.append((task, group_id, stratum, split))
                counts[split] += 1
        ...
        counts = dict(development=0, holdout=0)
        for task, group_id, stratum, split in accepted:
    ...
    return freeze_suite(output, source, selected, audit, seed, reserved,
        scoring="Unmodified official evaluator; audited outcome-only subset excludes nonempty NL assertions, "
                "required action paths, handoffs and empty-trajectory passes.",
        grouping="One representative per related task group; official train/test membership is preserved.")


def prepare_official_suite(output, *, source, seed, domain, development, exclude_suites=()):
    """Freeze the retail round study or telecom release with every official test task."""
    if domain not in DOMAINS or development < 1:
        raise ValueError("Use an integrated domain and a positive development count")
    if domain == "retail" and exclude_suites:
        raise ValueError("The historical retail round design retains the exact requested official IDs")
    source, output = Path(source).resolve(), Path(output).resolve()
    revision = subprocess.check_output(["git", "-C", str(source), "rev-parse", "HEAD"], text=True).strip()
    if revision != UPSTREAM_REVISION or subprocess.run(["git", "-C", str(source), "diff", "--quiet", "HEAD"]).returncode:
        raise ValueError("Use the clean pinned upstream revision " + UPSTREAM_REVISION)
    ...
    groups = {task.id: digest("\n".join(sorted(item.id for item in group)).encode())[:16]
              for group in task_groups(tasks, domain) for task in group}
    ...
    def audit_task(upstream_id):
        if upstream_id not in audited:
            ...
            if domain == "telecom" and (criteria.nl_assertions or criteria.communicate_info or
                ...
                raise ValueError("Telecom release requires deterministic official criteria: " + upstream_id)
            ...
            full_reward = None if criteria.nl_assertions or RewardType.ACTION in criteria.reward_basis else empty.reward
            if domain == "telecom":
    ...
    if domain == "retail":
        development_ids = official["train"][:development]
        selection_rule = "Exact official train prefix in frozen order; no task exclusions or replacements."
    else:
    ...
    return freeze_suite(output, source, selected, audit, seed, reserved,
        scoring=("Unmodified official evaluator, including environment and required-action checks; telecom has no "
                 "NL assertions and needs no grader model calls." if domain == "telecom" else
                 "Unmodified official evaluator, including NL assertions and handoffs. The configured user model also "
                 "serves as the grader; its private requests and usage are separately identified and charged."),
