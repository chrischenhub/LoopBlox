...
本文保留为固定版本的设计依据，不是当前实验计划；下文“本轮”指这次源码分析。当前任务环境与下一阶段目标由 [README.md](README.md) 维护，BFS／DFS 运行协议见[实验 guideline](docs/random-search.md)。
...
分析时提出的一个**原生 harness 验证**方向是 Pi 的工具组调度：已读源码给出了现成的串行/并行选择和明确的工具兼容约束。这个原生对照无需假定 Pi 有独立 Planning / Review 阶段。下述设想未实施，也不属于当前 τ² BFS／DFS 或领域 study 的运行步骤。
