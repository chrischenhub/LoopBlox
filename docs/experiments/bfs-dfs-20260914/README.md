...
DFS 复用了相同采样器：已提交的 loop06 最后一次 `e0007`、loop08 最后一次 `e0005` 也都只有 4 道不同题。当前代码已改为每次评估不放回抽取 `n` 道不同题，超过题池大小则拒绝。该修复只适用于使用新实现的实验，不改写历史协议，也没有自动重跑本轮。
...
当前源码已包含后续工程整理；上述摘要并不把当前 checkout 当作当时的冻结实现。原 campaign 的 `implementation/` 保留了当时的错误采样实现。采用修正后的不放回规则需要另建实验；新实验步骤见 [随机搜索说明](../../random-search.md)。
